...
```{python}
data = pd.read_stata('crime_data.dta')
...
crimeTypePerState['STATE'] = crimeTypePerState["STATE"].apply(statetoabbr)
...
This visual immediately begs many questions and affirms a lot of the testaments across the US about DUI and drug abuse offenses. We can see that the major categories of offenses are DUI, Assault, Drug Abuse, Disorderly Conduct, and Vandalism. So the ones discussed before are that there is undisputedly a problem with DUI and Drug abuse in the United States. There is an interesting outlier here in both Wisconsin and Washington DC. My speculation about Washington DC is that since this data was collected in 2021, the most arrested offense being vandalism most likely has some connection to the Jan 6th Riots. Especially since the number of arrests is visibly negligible. We see an odd spike in drug abuse violations in Texas, which I initially thought was due to its high population; however, we don't see a similar quantity for California, which is the most populated state in America.

Furthermore, I would like to look at race distributions and the crime that they are arrested for, to see if we can connect race and the crime most likely to be arrested for, in hopes of examining some institutional bias.
...
ggplot(offense_race) + geom_pie(aes(slice='Percent', fill='OFFENSE'), stat='identity')+facet_grid(x ='Race/Ethnicity')
```

Looking at this chart we can see more of what we confirmed above, that Drug Abuse accounts for 58% of their arrests based on the most arrested offenses per state. But we can see more interesting data, like how DUI comprises 49% of Indigenous arrests. These pie charts beg the question of whether you can use race as an identifier for the type of crime arrested. 
...
After running a decision tree classifier to classify offenses based on a percentage of the races arrested for those crimes, my model had an accuracy of 0.6, and interestingly the precision for the drug abuse violations was 0.67 which would imply some bias in terms of who is getting arrested for drug-related crimes.
